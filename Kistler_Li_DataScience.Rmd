...
\subsection{Summarizing Monthly Median Rent}
...
We next created a log-transformed monthly median rent (lMMR) variable. This is preferred because it normalizes the distribution, which allows linear modeling assumptions to be met and also condenses the range of the data. You can see this in the two plots on the second row, along with the statistics below them.

\subsection{Building a Predicitive Model for Log-Transformed Monthly Median Rent}

After exploring and transforming MMR, the next step is to see what relationships exist between the transformed response variable and its potential predictors. An initial step for this is to produce scatterplots, such as the two seen below, in order to spot any obvious visual trends. For example, the percentage of owner occupied residences has a negative relationship with lMMR, while there does not appear to be a strong relationship between lMMR and the percentage of residences with no vehicles available.
...
In an effort to see how this compares to our predictors, we created similar maps according to several different variables, as seen in Appendix 1, and 
...
\section{Appendix 1: Alternative Graphs and Models}
...
CENSUSDATA %>% ggplot(aes(x=DP04_0058PE.Percent.No.vehicles.available,y=lMMR, color=10-URScore)) + geom_point() + scale_color_gradient(low="lightblue", high="navy")

CENSUSDATA %>% ggplot(aes(x=DP05_0046PE.Percent.Chinese,y=lMMR)) + geom_point() + geom_point(color='navy')
```
...
\section{Appendix 2: All Code Used}
